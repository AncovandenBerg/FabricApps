...
export default defineConfig(({ mode, command }) => {
  ...
  return {
    plugins: [
      react({
        tsDecorators: true,
        // @vitejs/plugin-react-swc's `tsDecorators` only enables decorator
        // *parsing*; SWC still emits the legacy (experimentalDecorators-style)
        // transform by default. Rayfin's `@entity()`/`@role()` decorators are
        // written against the standard (stage-3) decorators proposal and read
        // `context.metadata`, which only the modern transform populates.
        useAtYourOwnRisk_mutateSwcOptions(options) {
          options.jsc = options.jsc ?? {};
          options.jsc.transform = options.jsc.transform ?? {};
          options.jsc.transform.legacyDecorator = false;
