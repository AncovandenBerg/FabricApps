...
import type { CapacityCommandSchema } from '../../rayfin/data/schema';
...
let client: RayfinClient<CapacityCommandSchema> | null = null;
let localDev = false;

export function initRayfinClient(
  config: RayfinClientConfig
): RayfinClient<CapacityCommandSchema> {
  if (client) {
    throw new Error('Rayfin client is already initialized.');
  }
  client = new RayfinClient<CapacityCommandSchema>({
...
export function getRayfinClient(): RayfinClient<CapacityCommandSchema> {
  if (!client) {
    throw new Error(
