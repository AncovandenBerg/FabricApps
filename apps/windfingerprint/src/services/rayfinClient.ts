...
import type { WindFingerprintSchema } from '../../rayfin/data/schema';
...
let client: RayfinClient<WindFingerprintSchema, AppFunctionsSchema> | null = null;
let localDev = false;

export async function initRayfinClient(
  config: RayfinClientConfig
): Promise<RayfinClient<WindFingerprintSchema, AppFunctionsSchema>> {
  if (client) {
  ...
  client = new RayfinClient<WindFingerprintSchema, AppFunctionsSchema>({
...
export function getRayfinClient(): RayfinClient<WindFingerprintSchema, AppFunctionsSchema> {
  if (!client) {
    throw new Error(
